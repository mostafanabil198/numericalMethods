...
% Last Modified by GUIDE v2.5 03-May-2019 19:08:20
...
% handles    structure with handles and user data (see GUIDATA)
        ...
        table=[];
        for i=1:5
            table=[table; i (i+1) (i+2)];
        end
        
        [m,n] = size(table);
        sss = m
       setappdata(0, 't',table);
       

    %s = str2double(get(handles.test1 , 'string'));
    
    s = 5 * 2;
    setappdata(0,'evalue',s);
    result = struct();
    result.table = table;
    result.s = s;
    setappdata(0,'all',result);
